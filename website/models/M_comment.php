...
class M_comment extends M_comm {
    ...
    public function getCommentListByArtcleId($param) {
        $cols = 't_comments.id,title,nick_name as nickName,portrait,t_comments.content,t_comments.create_time';
        $where = array(
        ...
        $list = $this->db->select($cols)
            ...
            ->join('t_users_info', 't_comments.id=t_users_info.user_id', 'left')
            ->join('t_artcles', 't_artcles.id=t_comments.artcle_id', 'left')
            ->where($where)
